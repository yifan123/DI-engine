...
from ding.utils.data import default_collate, default_decollate, RandomShiftsAug
...
@POLICY_REGISTRY.register('drq_sac')
class DRQSACPolicy(SACPolicy):
    config = dict(
        # (str) RL policy register name (refer to function "POLICY_REGISTRY").
        type='drq_sac',
        # (bool) Whether to use cuda for network.
    ...
    def _init_learn(self) -> None:
        ...
        """
        # Init
        self._aug = RandomShiftsAug(self._cfg.learn.pad)
        super(DRQSACPolicy, self)._init_learn()
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        self._learn_model.train()
        self._target_model.train()
        obs = self._aug(data['obs'])
        next_obs = self._aug(data['next_obs'])
        reward = data['reward']
        done = data['done']
